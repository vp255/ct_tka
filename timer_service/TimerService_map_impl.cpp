...
int TimerService_map_impl::setTimer(int64_t alert_after_elapsed_ns, IListener* listener, void* user_ctx) {
  int id = counter++;
  timers.insert(TimerServiceElement<IListener>{current_time + alert_after_elapsed_ns, 0, listener, user_ctx, id, false});
  return id;
}

int TimerService_map_impl::setRepeatingTimer(int64_t interval, IListener* listener, void* user_ctx) {
  int id = counter++;
  timers.insert(TimerServiceElement<IListener>{current_time + interval, interval, listener, user_ctx, id, false});
  return id;
}

void TimerService_map_impl::cancelTimer(int timer_id) {
  for (auto it = timers.begin(); it != timers.end(); it = std::next(it, 1))
    if (it->id == timer_id) {
      timers.erase(it);
      return;
    }
...
void TimerService_map_impl::advanceClock(epoch_ns_t current_time_ns) {
  ...
  while (!timers.empty() && timers.begin()->alert_time <= current_time) {
    ...
    //if (canceled_timers.count(element.id)) { // maybe buggy
